...
async function loadConversation(id) {
    ...
    try {
        ...
        if (data.status) {
            const userSession = await fetchUserSession(); // Fetch current user session to identify sender

            data.messages.forEach(msg => {
                const isSender = msg.sender_id === userSession.user_id; // Check if current user is sender
                const messageElement = document.createElement('div');
                
                // Add `sent-message` class if the current user is the sender, else add `received-message`
                messageElement.classList.add('message', isSender ? 'sent-message' : 'received-message');
...
async function fetchUserSession() {
    // Add an implementation to fetch the logged-in user's session info
    return {
        user_id: "<?php echo $_SESSION['user_id']; ?>" // Use PHP to output the user ID directly into JavaScript
    };
}
